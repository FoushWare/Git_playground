...
- Commit: `fa7d647` - "Add demo commands script for easy reference"

### Current (main)
- **Development branch**
- State: Latest from QC with documentation
- All features from QC plus demo materials
...
git checkout QC
git merge-base --is-ancestor 28ca1c5 HEAD && echo "YES - deployed to QC" || echo "NO - not in QC"

# Check if Apple Pay removal is in UAT  
git checkout UAT
git merge-base --is-ancestor 28ca1c5 HEAD && echo "YES - deployed to UAT" || echo "NO - not in UAT"
```
...
1. **Feature developed** → Apple Pay added in feature branch
2. **Deployed to UAT** → v1.1.0 released for testing
3. **Issues found** → Apple Pay has compatibility problems
4. **Hotfix created** → Apple Pay removed in hotfix branch
